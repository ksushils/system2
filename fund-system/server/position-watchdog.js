...
// A missing Capital deal is never treated as closed from position absence alone.
// The existing close route resolves /history/activity by exact deal id and
// refuses the mutation when the authoritative close evidence is unavailable.
export async function reconcileAbsentCapitalTrade(trade,{fetchImpl=fetch,apiBase='http://127.0.0.1:3210',scannerKey=process.env.SCANNER_API_KEY}={}){
 if(!scannerKey)return {attempted:false,reason:'SCANNER_KEY_MISSING'};
 try{
  const response=await fetchImpl(`${apiBase}/api/trade/close`,{method:'POST',headers:{'Content-Type':'application/json','x-scanner-key':scannerKey},body:JSON.stringify({scanner:trade.scanner,deal_id:trade.deal_id,ticker:trade.ticker,action:'FULL_EXIT'})});
  const body=await response.json().catch(()=>({}));
  return {attempted:true,closed:response.ok&&body.status==='ok'&&body.found===true,http_status:response.status,risk_released:body.risk_released||null,reason:body.reason||body.message||null};
 }catch(error){return {attempted:true,closed:false,reason:`RECONCILIATION_REQUEST_FAILED:${error.message}`};}
}
...
export async function runDealDrift(db,{sendTelegramAlert,save,simulate,journalEvent}={}){
 ...
 for(const action of actions){if(action.type==='DEAL_ID_DRIFT')await journalEvent?.('id_drift',{scanner:action.broker==='binance'?'crypto':null,ticker:action.ticker,payload:action});if(action.type==='DEAL_ID_RECONCILED')await journalEvent?.('deal_id_reconciled',{ticker:action.ticker,payload:action});if(action.type==='EXTERNAL_CLOSE')await journalEvent?.('external_close',{ticker:action.ticker,payload:action});}
