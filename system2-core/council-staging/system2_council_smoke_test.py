...
def main() -> None:
    ...
    for row in rows:
        ...
        raw = {
            "claude": call_claude(SYSTEM_PROMPT, user) if os.environ.get("ANTHROPIC_API_KEY") else None,
            "gpt": call_gpt(SYSTEM_PROMPT, user) if os.environ.get("OPENAI_API_KEY") else None,
            "gemini": call_gemini(SYSTEM_PROMPT, user) if os.environ.get("GEMINI_API_KEY") else None,
        }
        parsed = [
            {**parse_model_json(raw["claude"], vals["ticker"]), "_model": "claude"},
