...
def calculate(row: dict[str, Any]) -> dict[str, Any]:
    ...
    # Configurable finalist threshold (user spec: 55; using 35 during initial
    # data accumulation — raise to 55 once confirmation signals prove value)
    config = load_config()
    finalist_threshold = config.get("trade_quality", {}).get("finalist_threshold", 35)
